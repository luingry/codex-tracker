...
Assert(indicatorTemplate.Contains("x:Name=\"AgentWorkSpinner\" Data=\"M10,1 A9,9 0 1 1 3.64,3.64\" Stroke=\"#FFFFFF\" StrokeThickness=\"1.6\"", StringComparison.Ordinal) && indicatorTemplate.Contains("Opacity=\".60\" Visibility=\"Collapsed\"", StringComparison.Ordinal) && indicatorTemplate.Contains("<BlurEffect Radius=\"2\" />", StringComparison.Ordinal) && indicatorTemplate.Contains("<DataTrigger Binding=\"{Binding IsWorkAnimationEnabled}\" Value=\"True\">", StringComparison.Ordinal) && indicatorTemplate.Contains("x:Name=\"AgentWorkSpinnerStoryboard\"", StringComparison.Ordinal) && indicatorTemplate.Contains("Storyboard.TargetName=\"AgentWorkSpinnerRotation\" Storyboard.TargetProperty=\"Angle\" To=\"-360\" Duration=\"0:0:1.10\"", StringComparison.Ordinal) && indicatorTemplate.Contains("<StopStoryboard BeginStoryboardName=\"AgentWorkSpinnerStoryboard\" />", StringComparison.Ordinal), "active agents display a blurred 60-percent white glow arc which rotates counterclockwise only while work animation is allowed and stops cleanly for reduced motion");
...
Assert(agentListTemplate.Contains("HorizontalOffset=\"-149\" VerticalOffset=\"-10\"", StringComparison.Ordinal) && agentListTemplate.Contains("<Border Padding=\"15\" Background=\"Transparent\">", StringComparison.Ordinal) && agentListTemplate.Contains("x:Name=\"AgentListWrapper\" Width=\"288\" MaxHeight=\"350\" Padding=\"0,8\"", StringComparison.Ordinal) && agentListTemplate.Contains("<Border.Effect><DropShadowEffect BlurRadius=\"12\" ShadowDepth=\"3\" Opacity=\".28\" Color=\"#151A18\" /></Border.Effect>", StringComparison.Ordinal) && agentListTemplate.Contains("x:Name=\"AgentRow\" Margin=\"0,2\"", StringComparison.Ordinal) && agentListTemplate.Contains("<Border Margin=\"{Binding Indent}\" Padding=\"15,6\">", StringComparison.Ordinal), "agent wrapper has clearance for both its blur and displaced shadow while compensated offsets preserve popup placement and row interactions span its full width");
